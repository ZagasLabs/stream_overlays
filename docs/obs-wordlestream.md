# WordleStream OBS Setup

1. Generate a private URL with `npm run url:wordle -- --session SESSION_ID --production`. Never show or screenshot the complete URL.
2. Add an OBS Browser Source using `https://zagaslabs.github.io/stream_overlays/wordlestream/#session=SESSION_ID`.
...
Viewers send `!w CRANE` or `!word CRANE`. Change language with `lang=es`; Spanish input supports accents, with accented-vowel folding enabled by default and `ñ` preserved. Use `command=!guess` for a custom command and `userCooldown`/`globalCooldown` values in seconds.

Broadcasters and moderators can run `!word start`, `pause`, `resume`, `reset`, `reveal`, or `new` when SSN provides verified role metadata. Add explicit stable identities with `admins=twitch:name,kick:name,youtube:channel-id` when needed. A display-name role string is not sufficient.
