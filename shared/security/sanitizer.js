...
export function sanitizeMessageParts(value) {
  ...
  for (const match of html.matchAll(IMG_TAG_RE)) {
    addTextPart(parts, html.slice(cursor, match.index));
  ...
  if (parts.length === 0) addTextPart(parts, html);
  return {
...
function sanitizeTokenList(value, allowedTokens) {
  ...
  return String(value || "").split(/\s+/).filter((token) => allowed.has(token)).join(" ");
}
