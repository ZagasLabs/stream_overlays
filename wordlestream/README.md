...
An original cooperative stream word game. It is not affiliated with or derived from NYT Wordle and uses no game branding, assets, sounds, or presentation.

## Local use

Run `npm run mock:wordle`, then open `http://127.0.0.1:8765/wordlestream/#mock=1&debug=1`. Live use reads the SSN Session ID only from `#session=SESSION_ID`. Viewers submit `!w CRANE` or `!word CRANE`; use `command=!guess` to replace the accepted command list.

State is stored under `wordlestream:v1:LANG:LENGTH`. The key and value never contain the SSN Session ID. Clear the Browser Source cache/site data to discard a saved round, or use an authorized `!word reset` command.
...
Possible answers are a smaller, manually curated project vocabulary released under CC0-1.0. They are intentionally separate from the broader accepted-guess lists. Accented vowels can be preserved or folded; `ñ` remains distinct in both modes. No NYT/Wordle answer or guess list was used.
...
See the root README and `docs/obs-wordlestream.md` for full configuration and OBS setup.
