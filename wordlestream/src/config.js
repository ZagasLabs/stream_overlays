...
export const DEFAULT_WORDLE_CONFIG = Object.freeze({
...
export function parseWordleConfig({ hash = "", search = "", hostname = "", mediaReduceMotion = false, allowQueryFallback } = {}) {
  ...
  return {
    session,
    lang: parseEnum(get("lang"), ["en", "es"], DEFAULT_WORDLE_CONFIG.lang),
    commands,
    maxAttempts: parseIntRange(get("maxAttempts"), DEFAULT_WORDLE_CONFIG.maxAttempts, 3, 10),
    wordLength: parseIntRange(get("wordLength"), DEFAULT_WORDLE_CONFIG.wordLength, 4, 8),
    userCooldown: Math.round(parseFloatRange(get("userCooldown"), DEFAULT_WORDLE_CONFIG.userCooldown / 1000, 1, 120) * 1000),
    globalCooldown: Math.round(parseFloatRange(get("globalCooldown"), DEFAULT_WORDLE_CONFIG.globalCooldown / 1000, 0, 30) * 1000),
    maxQueue: DEFAULT_WORDLE_CONFIG.maxQueue,
    accents: parseEnum(get("accents"), ["fold", "preserve"], DEFAULT_WORDLE_CONFIG.accents),
    admins: parseAdmins(get("admins")),
    sound: parseBool(get("sound"), DEFAULT_WORDLE_CONFIG.sound),
    volume: parseFloatRange(get("volume"), DEFAULT_WORDLE_CONFIG.volume, 0, 0.5),
    accent: parseColor(get("accent"), DEFAULT_WORDLE_CONFIG.accent),
...
export function parseWordleConfigFromLocation(location = window.location) {
  return parseWordleConfig({ hash: location.hash, search: location.search, hostname: location.hostname, mediaReduceMotion: prefersReducedMotion() });
}

export function parseCommands(value) {
  if (!value) return [...DEFAULT_WORDLE_CONFIG.commands];
  const commands = String(value).split(",").map((item) => normalizeIdentityToken(item, 16).replace(/\s+/g, ""))
    .filter((item) => /^[!/.][a-z0-9]{1,12}$/.test(item));
  return [...new Set(commands)].slice(0, 4).length ? [...new Set(commands)].slice(0, 4) : [...DEFAULT_WORDLE_CONFIG.commands];
}
