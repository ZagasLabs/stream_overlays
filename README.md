...
- SSN connection: confirm the extension/app is enabled, the source chat is open, and platform WebSocket mode is enabled for full Twitch/Kick/YouTube events. For Twitch, keep exactly one dedicated WebSocket source connected as the broadcaster and turn off global **Hide stream events** plus any `new_follower` specific filter; normal DOM chat cannot produce follows. P2P and channel 4 are automatic; `npm run test:ssn-live` performs an optional network check with a random disposable room. The platform WebSocket source mode and overlay transport are separate layers. In debug mode, **LOCAL · render follow** verifies this overlay without claiming that SSN emitted an event.
